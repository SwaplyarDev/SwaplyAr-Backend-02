...
async reupload(
  ...
  const [frontImageUrl, backImageUrl, selfieImageUrl] = await Promise.all([
    this.cloudinaryService.uploadFile(files.document_front[0].buffer, folder, `front_${userId}`),
    this.cloudinaryService.uploadFile(files.document_back[0].buffer, folder, `back_${userId}`),
    this.cloudinaryService.uploadFile(files.selfie_image[0].buffer, folder, `selfie_${userId}`),
  ]);
