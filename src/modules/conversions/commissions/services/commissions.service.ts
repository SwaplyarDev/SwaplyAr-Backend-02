...
export class CommissionsService {
  calculateCommission(amount: number, fromPlatform: string, toPlatform: string): CommissionResponseDto {
    console.log('Checking rule for:', fromPlatform, '->', toPlatform);
    const rule = COMMISSION_RULES.find(
      (r) => r.from=== fromPlatform && r.to === toPlatform,
    );
  ...
  if (
    fromCurrencyInPlatform !== fromCurrency ||
    toCurrencyInPlatform !== toCurrency
  ) {
    return { valid: false };
  }
