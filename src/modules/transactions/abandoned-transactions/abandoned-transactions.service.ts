...
export class AbandonedTransactionsService {
  ...
  async create(createAbandonedTransactionDto: CreateAbandonedTransactionDto): Promise<AbandonedTransaction> {
    try {
      if (!createAbandonedTransactionDto.email || !createAbandonedTransactionDto.phone_number || 
          !createAbandonedTransactionDto.first_name || !createAbandonedTransactionDto.last_name) {
        this.logger.error('Datos incompletos en la creación de transacción abandonada');
        throw new BadRequestException('Todos los campos son requeridos');
      }

      const abandonedTransaction = this.abandonedTransactionRepository.create(createAbandonedTransactionDto);
      const savedTransaction = await this.abandonedTransactionRepository.save(abandonedTransaction);
      
      this.logger.log(`Transacción abandonada creada con ID: ${savedTransaction.abandoned_transaction_id}`);
  ...
  async update(id: string, updateData: Partial<CreateAbandonedTransactionDto>): Promise<AbandonedTransaction> {
    try {
      const abandonedTransaction = await this.findOne(id);
      
      Object.assign(abandonedTransaction, updateData);
      
      const updatedTransaction = await this.abandonedTransactionRepository.save(abandonedTransaction);
